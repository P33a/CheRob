...
uses
  ...
  Grids, StdCtrls, IniPropStorage, SdpoVideo4L2, VideoDev2, math;
...
type
  ...
  private
    ...
    function SafeGetFeatureValue(i: integer): integer;
...
function TFCamFeatures.SafeGetFeatureValue(i: integer): integer;
begin
...
begin
  ...
  newvalue := min(newvalue, Video.UserControls[curFeatureIdx].maximum);
  newvalue := max(newvalue, Video.UserControls[curFeatureIdx].minimum);
...
begin
  ...
  updatingHeader := 1;
  SBFeatureValue.SetParams(value, Video.UserControls[curFeatureIdx].minimum, Video.UserControls[curFeatureIdx].maximum);
  updatingHeader := 0;
end;
